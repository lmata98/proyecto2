...
namespace proyecto2
{
    public partial class registrarPenales : Form
    {
        ...
        private int cantidadPenales(string equipo)
        {
            /*  string oradb = "DATA SOURCE=localhost:1521/XE;USER ID=SYSTEM; Password = toma;";
              int aux=0;
              OracleConnection conn = new OracleConnection(oradb); // C#
            ...
            cmd.CommandText = "select idequipo from penales where idequipo = '" + equipo + "' AND IDPARTIDO = " + idPartido;
        ...
        private void registrar_Click(object sender, EventArgs e)
        {
            ...
            int auxC = cantidadPenales(equipo);
            MessageBox.Show("cantidad         " + auxC.ToString());
            if (ExJ1 || autoGol)
            {
                cont++;
                string instruccion = "insert into penales values( " + auxC + ", '" + Anotador + "','" + equipo + "'," + idPartido + ",'" +anotado + "' )";
